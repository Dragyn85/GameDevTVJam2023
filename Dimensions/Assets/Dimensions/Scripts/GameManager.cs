	...
	void Update ()
	{
		switch (_gameState)
		{
			...
			case GameStates.BeatLevel:
				...
				if (_backgroundMusicAudioSource.volume<=0.0f)
				{
					float rawVolume = GameServiceLocator.GetInstance().GetService<AudioManager>().GetSoundNormalizedParameterValue("SFXVolume");

                    AudioSource.PlayClipAtPoint (_beatLevelSFX,_backgroundMusicAudioSource.transform.position, rawVolume);
